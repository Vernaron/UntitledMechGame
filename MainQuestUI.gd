...
func get_quests()->void:
	current_quests.append_array(PlayerInfo.active_save_data["completed_main_quest"])
	for completed_quest in current_quests:
		var current_quest : QuestData.Quest = QuestData.main_quests[completed_quest]
		for next_quest in current_quest.main_quest_unlocks:
			if current_quests.find(next_quest)==-1:
...
func _on_item_clicked(index: int, _at_position: Vector2, _mouse_button_index: int) -> void:
	...
	Signals.display_quest_data.emit(QuestData.main_quests[selected_quests])
