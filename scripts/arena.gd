...
func build_player():
	...
	PlayerInfo.active["legs"] = ItemData.legs["strider_1"]
	PlayerInfo.active["body"] = ItemData.bodies["strider_1"]
	PlayerInfo.active["weapons"] = [ItemData.weapons["bolter"], ItemData.weapons["bolter"]]
	active_player.construct(
		"Player", $mainCamera, self
	)
